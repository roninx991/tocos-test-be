...
describe('sendTransaction', () => {
    ...
    it("throws an error if 'from' account and 'to' account are same", async () => {
        ...
        await expect(sendTransaction({ from: 'fromUser', to: 'fromUser', amount: 1000})).rejects.toThrowError("'from' acount and 'to' account cannot be the same.");
    });    
    ...
    it("creates a new transaction with valid amount", async () => {
        ...
        saveTransactionMock.mockResolvedValueOnce({ _id: randomUUID().toString(), from: 'fromUserId', to: 'toUserId', amount: '1000' })

        const result = await sendTransaction({ from: 'fromUsername', to: 'toUsername', amount: 1000 });
        ...
        expect(userRepository.findUserByIdOrUsername).toHaveBeenCalledWith('fromUsername');
        expect(userRepository.findUserByIdOrUsername).toHaveBeenCalledWith('toUsername');
