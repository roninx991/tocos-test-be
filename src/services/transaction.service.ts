...
export const sendTransaction = async (body: any) => {
  try {
    ...
    const fromUser = await findUserByIdOrUsername(body.from);
    const toUser = await findUserByIdOrUsername(body.to);
    ...
    const transaction: Transaction = {
      ...
      from: fromUser._id,
      to: toUser._id,
      amount: body.amount.valueOf().toString(),
    };

    await updateBalance(
      fromUser._id,
      BigInt(fromUser.balance).valueOf() - BigInt(transaction.amount).valueOf()
    );
    await updateBalance(
      toUser._id,
      BigInt(toUser.balance).valueOf() + BigInt(transaction.amount).valueOf()
    );
